...
void main()
{
	...
	// new position are calculate using previous velocity
	vec3 newPosition = velocity * Time.delta + position;
	...
	for (uint i = 0; i < ObjectCount; ++i)
	{
		if (id != i)
		{
			...
			acceleration += (GxMScale * normalize(distance) / (dot(distance, distance) + 1)); // add 1 to avoid large acceleration
		}
	}

	vec3 velocityDelta = acceleration * Time.delta;
